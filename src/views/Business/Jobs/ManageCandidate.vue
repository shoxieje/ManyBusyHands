<template>
    <div class="manage-candidate-top text-primary">
        <div class="manage-candidate-header mw-60">
            <h1 class="title">
                Candidates for {{ job_ad.title }} at {{ job_ad.suburb + " " + job_ad.postcode + " " + job_ad.state }} 
            </h1>
            <div class="subtitle">
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            // if state is loaded
            if(this.activeJobList.length !== 0 || this.inactiveJobList.length !== 0) {
                // check active ad
                for(let x of this.activeJobList) {
                    if(x.random_url_job_id == jobID) 
                        this.job_ad = x
                ...
                // state is not loaded
                await axios.get(`http://localhost:8081/jobAd/getJobAdByUUID/${jobID}`).then(
                    (result) => {
            ...
            await axios.get(`http://localhost:8081/candidate/${jobID}`).then((result) => {

                for(const candidate of result.data) {


                    axios.get(`http://localhost:8081/jobSeekerUser/searchByEmail/${candidate.user_email}`).then(
