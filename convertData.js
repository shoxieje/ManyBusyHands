...
async function processByLine() {
    ...
    for await (const line of rl) {
        ...
        if(line.includes('/')) {
            ...
            for(let x of tempActivities) {
                ...
                if(xx.length >= 2) {
                    if((xx[0].startsWith("Au") || xx[0].startsWith["au"]) && (xx[1].startsWith("S") || xx[1].startsWith("s"))) {
    ...
    for(const x of activity) {
        ...
        await axios.post(`http://localhost:8081/activity`, new URLSearchParams({
            activityName: x
        }), {
...
function addToActivity(value) {
    ...
    const splitWords = value.split(" ").filter(e => {
        return e !== '' && e !== '.'
    }).map((word) => {
